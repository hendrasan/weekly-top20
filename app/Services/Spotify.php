    ...
    try {
      ...
      if (!empty($latest_chart)) {
        ...
        foreach ($top20_tracks->items as $key => $item) {
          ...
          $track_peak_position = Chart::where('user_id', $user->id)->where('track_spotify_id', $item->id)->min('position');
          ...
          $pushed_item = array(
            ...
            'periods_on_chart' => Chart::where('user_id', $user->id)->where('track_spotify_id', $item->id)->count() + 1,
